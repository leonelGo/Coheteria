...
def Ec_Presion2(t, CI, p):
        ...
        #print(f'altura {y}')
        #print(f'Pe {P_e}', f'Po {Po}', f'Pa {P_atm}')
        C_f = N_throat*N_div*N_noz*(N_skin*Cf2_(P_e, (Po + P_atm), P_atm, At, Ae, k)+(1-N_skin)) # agregar las eficiencias a los parametros

        #print(C_f)
        F = C_f*At*(Po + P_atm)
        #dm_t = mdot(At, Po, k, To, cte.R/M)
        #print(f'masa {m1-(dmdt)*t}')
        #print(f'masa m {m}')
        #print(f'masa dm {(dmdt)*t}')
        dxdt = u
        dudt = (1/(m1-(dmdt)*t))*(F 
...
# Diametros
Dc = 1.875*25.4  #mm Diametro de la camara 
Df = 3 # in, diametro fuselaje
Dt = 14 # mm, de pulgadas a mm [(in)*(mm/in)]
Dp =  .8*25.4 # mm, de pulgadas a mm [(in)*(mm/in)] # Diametro del port
...
P0 = 830 #vPe*((k+1)/2)**(k/(k-1)) #psi, presion de la camara objetivo
...
L_0 = (3*Dc+Dp)/2
N = 4 # round(4*mp0/L_0/rho/(Dc**2-Dp**2)/np.pi) # Número de segmentos del propelente
...
#---------------------------------------------------------
    if PresionCambiante:
        ...
        #print(f'presion camara {len(Presion_camara)}')
        ...
        C_f = N_throat*N_div*N_noz*(N_skin*Cf_(P_e + P_a, (Presion_camara + P_a), P_a, A_t, A_e, k)+(1-N_skin))
        F = C_f*At*(Presion_camara + P_a)

            # Verificar que las longitudes coincidan
        if len(t_thrust) != len(F):
            print(len(t_thrust), len(F))
            raise ValueError("The length of `t_thrust` doesn't match the length of `F`")
    ...
    if PresionCambiante == True: PresionAtmosferica = P_atm 
    else:   PresionAtmosferica = (np.ones(len(t_thrust))*P_a) 
    ...
    P_atm = Propiedades_Atm(y_sol1)[0]*1e-6
    P_e = P_Po(Presion_camara + PresionAtmosferica, Me, k)

    C_f = N_throat*N_div*N_noz*(N_skin*Cf_(P_e + PresionAtmosferica, (Presion_camara + PresionAtmosferica), P_atm, A_t, A_e, k)+(1-N_skin))
    F = C_f*At*(Presion_camara + P_atm)
...
Cf_2 = N_throat*N_div*N_noz*(N_skin*Cf2_(P_e, (Presion_camara + P_atm), P_atm, A_t, A_e, k)+(1-N_skin))

Cf_opt2= Cf2_(P_e, (Presion_camara + P_atm) , P_e, At, Ae, k)
#Cf_opt3 = C
Otro = (P_e-P_atm)/(Presion_camara + P_atm)*AeAt
...
#aaa = np.sqrt(2 * k**2 / (k - 1) * (2 / (k + 1))**((k + 1) / (k - 1))*(1-(P_e/(Presion_camara+P_a))**((k-1)/k))) + (P_e-P_a)*A_e/((Presion_camara+P_a)*A_t)

#plt.figure(figsize=(7,7))
#plt.plot(t_thrust, (P_e/Presion_camara)**((k-1)/k), '-r')
#plt.plot(t_thrust, (P_e-P_a)*Ae/((Presion_camara+P_a)*At), '-b')
#plt.plot(t_thrust, aaa, '--k')
#plt.ylim(bottom=-5, top = 3)
...
po_array = (Presion_camara + P_atm) * 145.038  # Conversion from Pa to psi
pa_array = P_atm * 145.038  # Conversion from Pa to psi
...
# Función para encontrar pe que cumple Cf >= 0
def find_pe_brentq(po, pa, At, Ae, k):
    # Definimos la función a encontrar la raíz
    def func(pe):
        return Cf2_(pe, po, pa, At, Ae, k)
    
    # Usamos brentq para encontrar la raíz en el intervalo [pa, po]
    try:
        pe_solution = brentq(func, pa, po)
        return pe_solution
    except ValueError:
...
for po, pa in zip(po_array, pa_array):
    pe_min = find_pe_brentq(po, pa, At, Ae, k)
...
P_re = (np.array(P_e + P_atm))*145.038 # Mpa
#Pe_opt = (P_atm - Presion_camara*coeficiente_empuje_optimo(k, Me)/AeAt)*145.038
if PresionCambiante == True: PresionAtmosferica = P_atm*145.038 
else:   PresionAtmosferica = (np.ones(len(t_thrust))*Pa) 
