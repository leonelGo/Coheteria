...
P0 = 1080 #vPe*((k+1)/2)**(k/(k-1)) #psi, presion de la camara objetivo
...
# %%
Cd =  0.5 # 0.45 #0.48 # coeficiente de arrastre

#102.26 
Dc = 1.875*25.4 #(8*3/8)*25.4 #2*25.4 #mm Diametro de la camara 
Df = 3 # in, diametro fuselaje

Dt = 12 #(16/16/np.sqrt(2))*25.4 # mm, de pulgadas a mm [(in)*(mm/in)]
L_t = 3 # mm
L_Dt = L_t/Dt
...
Dp =  .7*25.4 #(16*1/16)*25.4 # mm, de pulgadas a mm [(in)*(mm/in)] # Diametro del port
...
Tolerancia = 1 # Error de la altura alcanzada en metros
...
while np.abs(h0-h[i]) > Tolerancia:
    ...
    # %%
    mdot_ = mdot(A_t, Presion_camara, k, To, cte.R/M)
...
plt.plot(t_sol2, y_sol2, 'r-', label = 'Fase propulsada' ) # posición en y 
...
P_ro = (P_Po(Presion_camara*145.038 + 14.69, Me, k)) # kp
P_re = (np.array(P_e))*145.038/1e6 # kpa
