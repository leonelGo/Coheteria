...
Creador = 'Nakka' # Posibles: Nakka, UDEG SPACE, MIT, Altum
...
P0 = 500 #vPe*((k+1)/2)**(k/(k-1)) #psi, presion de la camara objetivo
...
L_0 = (3*Dc+Dp)/2
N = 3 #round(4*mp0/L_0/rho/(Dc**2-Dp**2)/np.pi) # Número de segmentos del propelente
...
if  Vuelo:  
    ...
    while np.abs(h0-h[i]) > Tolerancia:
        ...
        print(f'N granos {round(4*m_p[i]/(L_0)/(rho*1e-6)/(Dc**2-Dp**2)/np.pi) }, {4*m_p[i]/L_0/rho/(Dc**2-Dp**2)/np.pi}')
        fase2_activa = False
        tb = 0
    ...
    #---------------------------------------------------------
        if PresionCambiante == True:
            ...
            Solucion_1 = solve_ivp(Ec_Presion2, t_span, CI1, args=(p_P,), events = event_pressure_zero2,
                                    t_eval=t_eval, rtol=1e-6, atol=1e-4)
        ...
        if np.abs(h0-h[i+1]) > Tolerancia:
            ...
            if i > 150:
                ...
                Inc_M = m_p[i]*0.3*np.sign((h0-h[i+1])/h[i+1])#/h[i+1]*(h0-h[i+1])
...
print(f'Eficicia de tobera {N_efi:.3f}')
